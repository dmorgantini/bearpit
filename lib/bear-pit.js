export class BearPit {
  ...
  generateFightDuration(fighter1, fighter2) {
    // Generate fight duration based on configured average and variance
    const avgDuration = this.config.averageFightDurationSeconds;
    const variance = this.config.fightDurationVariance;
    const fightDurationVariancePerLevel = this.config.fightDurationVariancePerLevel || 0;

    const minVariance = fightDurationVariancePerLevel * Math.abs(fighter1.level - fighter2.level) + variance;
    
    // Generate random duration with normal-ish distribution
    // Using simple approach: average ± random variance
    const minDuration = Math.max(10, avgDuration - minVariance); // Minimum 10 seconds
  ...
  startNewFight(queueManager) {
    ...
    const fightDuration = this.generateFightDuration(fighter1, fighter2);
