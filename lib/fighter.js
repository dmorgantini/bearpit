export class Fighter {
  constructor(name, level) {
    ...
    this.luckyFights = 0; // Fights where opponent had <10% win chance (fighter had >90% chance)
  ...
  getWinRate() {
    ...
    return (this.totalWins / this.totalFights) * 100;
  }
  reset() {
    this.currentStreak = 0;
    this.longestStreak = 0;
  ...
  recordFight(winProbability, opponentWinProbability) {
    this.totalFights++;

    // Calculate the "pure" win probability excluding simuls
    // This gives us the actual head-to-head advantage
    const totalNonSimulProbability = winProbability + opponentWinProbability;
    const pureWinProbability = totalNonSimulProbability > 0 ?
      winProbability / totalNonSimulProbability : 0.5;

    if (pureWinProbability < 0.1) { // Less than 10% chance in head-to-head
      this.unluckyFights++;
    }

    if (pureWinProbability > 0.9) { // Greater than 90% chance in head-to-head
      this.luckyFights++;
    }
  }
  ...
  getLuckyPercentage() {
    if (this.totalFights === 0) return 0;
    return (this.luckyFights / this.totalFights) * 100;
  }
