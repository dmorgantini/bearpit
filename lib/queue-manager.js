export class QueueManager {
  ...
  // NEW: Check if there are enough fighters for a fight
  canStartFight(pitId, currentTime, hasChampion = false) {
    const requiredFighters = hasChampion ? 1 : 2; // Need 1 challenger if champion exists, 2 if no champion
    
    if (this.useShortestQueue) {
      const availableFighters = this.queues[pitId].filter(fighter => 
        fighter.isAvailableAt(currentTime)
      ).length;
      return availableFighters >= requiredFighters;
    } else {
      const availableFighters = this.sharedQueue.filter(fighter => 
        fighter.isAvailableAt(currentTime)
      ).length;
      return availableFighters >= requiredFighters;
    }
