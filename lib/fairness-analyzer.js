...
export class FairnessAnalyzer {
  ...
  analyzeFairness(timeOptions = [10, 15, 20, 25], pitOptions = null, iterations = 10) {
    ...
    console.log(`Analyzing ${isRetirementMode ? 'RETIREMENT MODE' : 'TRADITIONAL'} fairness for ${this.baseFighters.length} fighters across ${timeOptions.length} time options, ${pitOptions.length} pit options with ${iterations} iterations each...`);
    ...
    for (const time of timeOptions) {
      for (const pits of pitOptions) {
        ...
        // Average the award analysis metrics
        const numericMetrics = [
          'totalEliminations',
          'eliminationsByAwardWinners',
          'eliminationsWhileAwardWinner',
          'totalFighters',
          'eligibleFighters',
          'ineligibleFighters',
          'awardWinners',
          'eligibleButDidntWin',
          'awardWinnerEliminationRate',
          'eligibilityRate',
          'awardSuccessRate'
        ];
        ...
        numericMetrics.forEach(metric => {
          const values = configResults
            .map(run => run.result.awardAnalysis?.[metric])
            .filter(val => val !== undefined);
          
          if (values.length > 0) {
            awardAnalysis[metric] = values.reduce((sum, val) => sum + val, 0) / values.length;
          }
        });

        // For object metrics, use the most recent run's data
        const lastRun = configResults[configResults.length - 1];
        if (lastRun.result.awardAnalysis) {
          if (lastRun.result.awardAnalysis.eliminationsByLevel) {
            awardAnalysis.eliminationsByLevel = lastRun.result.awardAnalysis.eliminationsByLevel;
          }
          if (lastRun.result.awardAnalysis.mostAffectedFighters) {
            awardAnalysis.mostAffectedFighters = lastRun.result.awardAnalysis.mostAffectedFighters;
          }
        }
